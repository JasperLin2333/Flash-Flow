...
export default function AppPage() {
    ...
    useEffect(() => {
        if (chatId && flowId) {
            ...
            // Only if we are NOT currently executing AND NOT loading history (to prevent clearing state if URL update lags)
            if (!isExecuting && !isLoadingHistory) {
                console.log(`[AppPage] No chatId in URL, clearing state`);
                startNewConversation();
            }
        }
    }, [chatId, flowId, currentChatId, loadChatHistory, startNewConversation, isExecuting, isLoadingHistory]);
