...
function AppContent() {
    ...
    const loadChatHistory = useCallback(async (sessionId: string, skipUrlUpdate: boolean = false) => {
        ...
        try {
            ...
            let history = await chatHistoryAPI.getSessionMessages(sessionId);

            if (!history || history.length === 0) {
                // Fallback: Try to load as legacy message ID
                const legacyMsg = await chatHistoryAPI.getMessageById(sessionId);
                if (legacyMsg) {
                    console.log(`[LoadChat] Found legacy message: ${sessionId}`);
                    history = [legacyMsg];
                } else {
