"use client";
import { MousePointer2, Hand, Minus, Plus, Maximize, Play } from "lucide-react";
    ...
    <div className="flex items-center gap-1">
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <button className="text-xs font-medium text-gray-700 w-12 text-center select-none rounded-md hover:bg-gray-100 px-1 py-1">
            {zoomPct}%
          </button>
        </DropdownMenuTrigger>
  ...
  return (
    <>
      ...
      <div className="fixed bottom-8 left-8 z-10 flex items-center gap-1 bg-white shadow-xl border border-gray-200 rounded-full px-2 py-1.5 h-12">
        ...
        <ZoomControls zoomPct={zoomPct} zoomIn={zoomIn} zoomOut={zoomOut} zoomTo={zoomTo} fitView={fitView} />
        <Separator orientation="vertical" className="h-6 bg-gray-200" />
