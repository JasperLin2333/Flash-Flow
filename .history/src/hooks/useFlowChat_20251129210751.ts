...
export function useFlowChat({ flowId }: UseFlowChatProps) {
    ...
    const messagesRef = useRef<ChatMessage[]>([]); // Keep track of messages for "already loaded" check
    ...
    const loadSession = useCallback(async (sessionId: string) => {
        ...
        if (sessionId === activeSessionIdRef.current && messagesRef.current.length > 0) return;
    ...
    const startNewSession = useCallback(() => {
        ...
        // Just update URL. The useEffect in page.tsx will detect the change and call clearSession().
        // This prevents the "double click" race condition where state clears -> effect runs -> reloads old session -> URL updates.
