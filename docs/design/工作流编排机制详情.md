# 🔄 工作流编排机制详情

本文档详细描述 Flash Flow 工作流的底层编排机制，包括上下文管理、变量系统、执行引擎、分支控制和安全机制。

---

## 📦 FlowContext 上下文结构

工作流使用 `FlowContext` 作为全局上下文，采用**键值对映射**存储每个节点的输出数据：

```typescript
// 定义于 src/types/flow.ts

export interface FlowContextMeta {
  flowId?: string | null;   // 当前工作流 ID
  sessionId?: string;        // 会话 ID（用于对话记忆）
  nodeLabels?: Record<string, string>;  // nodeId → label 映射
}

export interface FlowContext {
  [nodeId: string]: Record<string, unknown> | FlowContextMeta | undefined;
  _meta?: FlowContextMeta;  // 元数据（以 _ 开头）
}
```

### 上下文示例

```
{
  "input_abc123": {
    "user_input": "帮我写一篇文章",
    "files": [],
    "formData": {}
  },
  "llm_def456": {
    "response": "这是生成的文章内容..."
  },
  "tool_ghi789": {
    "formatted": "2025-12-15 12:00:00",
    "timestamp": 1734220800000,
    "timezone": "Asia/Shanghai"
  },
  "_meta": {
    "flowId": "flow_xyz",
    "sessionId": "session_123",
    "nodeLabels": {
      "input_abc123": "用户输入",
      "llm_def456": "文章生成",
      "tool_ghi789": "获取当前时间"
    }
  }
}
```

### 上下文数据访问工具

系统提供标准化的上下文访问函数，定义于 [contextUtils.ts](file:///Users/jasperlin/Desktop/product/flash-flow-saas/flash-flow/src/store/executors/contextUtils.ts)：

| 函数 | 功能 |
|------|------|
| `getUpstreamData(context)` | 获取第一个上游节点的输出数据 |
| `getUpstreamEntries(context)` | 获取所有上游节点的 `[nodeId, output]` 数组 |
| `extractTextFromUpstream(data)` | 智能提取文本内容 |
| `extractInputFromContext(context)` | 组合提取输入文本 |
| `extractOutputFromContext(nodes, context)` | 提取 Output 节点的完整输出（含附件） |

---

## 📊 节点输出标准格式

| 节点类型 | 主要输出字段 | 说明 |
|---------|-------------|------|
| **Input** | `user_input`, `files`, `formData` | 用户输入的文本、文件和表单数据 |
| **LLM** | `response` | AI 生成的回复内容（支持流式、JSON 模式） |
| **RAG** | `query`, `documents`, `citations`, `documentCount`, `mode` | 检索查询、文档块、引用及模式（fileSearch/multimodal） |
| **Tool** | 工具特定字段 | 根据工具类型不同，如 `formatted`, `results`, `content` 等 |
| **Branch** | `conditionResult`, `passed`, + 透传数据 | 条件判断结果及上游数据透传 |
| **Output** | `text`, `attachments` | 最终展示文本和附件列表 |
| **ImageGen** | `imageUrl` | AI 生成的图片 URL |

---

## 🔗 变量引用机制

### 支持的引用格式

| 格式 | 说明 | 示例 |
|------|------|------|
| `{{field_name}}` | 直接使用字段名（在所有上游节点中查找） | `{{user_input}}`, `{{response}}` |
| `{{节点名称.field_name}}` | 使用节点 label 前缀（**推荐**，更明确） | `{{获取当前时间.formatted}}` |
| `{{node_id.field_name}}` | 使用节点 ID 前缀 | `{{tool_abc123.formatted}}` |

### 变量收集流程

变量收集由 [variableUtils.ts](file:///Users/jasperlin/Desktop/product/flash-flow-saas/flash-flow/src/store/executors/utils/variableUtils.ts) 中的 `collectVariables()` 函数实现：

``mermaid
flowchart TD
    A[开始收集变量] --> B{是否调试模式?}
    B -->|是| C[使用 Mock 数据]
    C --> Z[返回变量映射]
    B -->|否| D[遍历全局 flowContext]
    D --> E[展开节点输出为变量<br/>跳过直接上游]
    E --> F[遍历直接上游 context]
    F --> G[展开直接上游输出<br/>覆盖同名变量]
    G --> H[添加自定义输出变量]
    H --> Z
```

### 变量收集算法

```typescript
function collectVariables(context, globalFlowContext, allNodes, mockData?) {
  const allVariables = {};
  
  // 1. 调试模式：直接使用 mock 数据
  if (mockData && Object.keys(mockData).length > 0) {
    Object.entries(mockData).forEach(([key, value]) => {
      allVariables[key] = String(value);
    });
    return allVariables;
  }
  
  // 2. 先从全局 flowContext 提取（较早执行的节点）
  for (const [nodeId, nodeOutput] of Object.entries(globalFlowContext)) {
    if (nodeId.startsWith('_')) continue;
    if (context[nodeId]) continue;  // 跳过直接上游（后面处理）
    
    const nodeLabel = findNodeLabel(nodeId, allNodes);
    flattenObject(nodeOutput, allVariables);            // {{fieldName}}
    flattenObject(nodeOutput, allVariables, nodeLabel); // {{节点名称.fieldName}}
    flattenObject(nodeOutput, allVariables, nodeId);    // {{nodeId.fieldName}}
    addCustomOutputs(nodeId, allVariables);
  }
  
  // 3. 最后从直接上游 context 提取（会覆盖全局同名变量）
  for (const [nodeId, nodeOutput] of Object.entries(context)) {
    if (nodeId.startsWith('_')) continue;
    
    flattenObject(nodeOutput, allVariables);            // 直接上游优先
    flattenObject(nodeOutput, allVariables, nodeLabel);
    flattenObject(nodeOutput, allVariables, nodeId);
    addCustomOutputs(nodeId, allVariables);
  }
  
  return allVariables;
}
```

### 变量优先级

1. **直接上游** context 中的节点输出（**最高优先级**）
2. **全局** flowContext 中的节点输出

### 变量未找到时的处理

- 替换为**空字符串**
- 控制台输出警告：`[PromptParser] 未找到变量: xxx，已替换为空字符串`

### flattenObject 展开规则

| 数据类型 | 处理方式 |
|---------|---------|
| `null` / `undefined` | 空字符串 `""` |
| 基础类型 | `String(value)` |
| 嵌套对象 | 递归展开，如 `obj.a.b` → `{{a.b}}` |
| 数组 | `JSON.stringify(array)` |
| `_` 开头的键 | **跳过**（内部字段） |

---

## 📝 文本提取优先级

`extractTextFromUpstream()` 函数按以下优先级智能提取文本：

```typescript
const TEXT_FIELD_PRIORITY = ['text', 'response', 'user_input', 'query'];
```

### 提取流程

``mermaid
flowchart LR
    A[输入数据] --> B{数据类型?}
    B -->|string| C[直接返回]
    B -->|非 object| D[String 转换]
    B -->|object| E[按优先级查找字段]
    E --> F{找到非空字段?}
    F -->|是| G[返回字段值]
    F -->|否| H{是 Branch 输出?}
    H -->|是| I[过滤元数据后再次查找]
    H -->|否| J[JSON stringify 兜底]
```

### Branch 节点特殊处理

```
const BRANCH_METADATA_FIELDS = ['conditionResult', 'passed', 'value'];

// Branch 输出中会过滤这些元数据字段后再提取文本
```

---

## ⚡ 并行执行引擎

系统使用基于**拓扑层级**的并行执行引擎，定义于 [executionActions.ts](file:///Users/jasperlin/Desktop/product/flash-flow-saas/flash-flow/src/store/actions/executionActions.ts) 和 [parallelExecutionUtils.ts](file:///Users/jasperlin/Desktop/product/flash-flow-saas/flash-flow/src/store/utils/parallelExecutionUtils.ts)。

### 执行流程概览

``mermaid
flowchart TD
    A[runFlow 开始] --> B[循环依赖检测]
    B --> C{有循环?}
    C -->|是| D[中止并报错]
    C -->|否| E[输入验证]
    E --> F{输入完整?}
    F -->|否| G[打开输入对话框]
    F -->|是| H[获取执行锁]
    H --> I{锁冲突?}
    I -->|是| J[警告并返回]
    I -->|否| K[重置执行状态]
    K --> L[初始化流式模式]
    L --> M[构建 FlowContext]
    M --> N[计算拓扑层级]
    N --> O[按层级并行执行]
    O --> P[释放执行锁]
    P --> Q[完成]
```

### 拓扑层级计算

``typescript
// src/store/utils/parallelExecutionUtils.ts

function calculateTopologicalLevels(nodes, edges): Map<nodeId, level> {
  // Level 0 = 入口节点（无上游依赖）
  // Level N = max(所有前置节点层级) + 1
  
  const calculateLevel = (nodeId) => {
    const incomers = getIncomers(node, nodes, edges);
    if (incomers.length === 0) return 0;  // 入口节点
    
    let maxUpstreamLevel = -1;
    for (const incomer of incomers) {
      maxUpstreamLevel = Math.max(maxUpstreamLevel, calculateLevel(incomer.id));
    }
    return maxUpstreamLevel + 1;
  };
}
```

### 层级并行执行

```
graph LR
    subgraph "Level 0"
        A[Input]
    end
    subgraph "Level 1"
        B[LLM-1]
        C[RAG]
    end
    subgraph "Level 2"
        D[Branch]
    end
    subgraph "Level 3"
        E[LLM-2]
        F[Tool]
    end
    subgraph "Level 4"
        G[Output]
    end
    A --> B
    A --> C
    B --> D
    C --> D
    D --> E
    D --> F
    E --> G
    F --> G
```

**执行规则**：
- 同层级节点**并行执行**（`Promise.allSettled`）
- 等待当前层级全部完成后，才执行下一层级
- 任一节点执行失败会记录错误并在当前层级结束后中止

### 执行锁机制

```
// 防止并发执行
if (get()._executionLock) {
  console.warn('[RunFlow] 执行已在进行中，请等待完成');
  return;
}
set({ _executionLock: true });

try {
  // 执行工作流...
} finally {
  set({ _executionLock: false });  // 确保释放锁
}
```

---

## 🌳 分支节点执行

Branch 节点使用**安全表达式求值器**，详见 [BranchNodeExecutor.ts](file:///Users/jasperlin/Desktop/product/flash-flow-saas/flash-flow/src/store/executors/BranchNodeExecutor.ts)。

### 支持的条件表达式

| 表达式类型 | 格式 | 示例 |
|-----------|------|------|
| **包含判断** | `节点名.字段.includes('关键词')` | `LLM1.response.includes('成功')` |
| **前缀判断** | `节点名.字段.startsWith('前缀')` | `Input.text.startsWith('查询')` |
| **后缀判断** | `节点名.字段.endsWith('后缀')` | `Tool.result.endsWith('.pdf')` |
| **相等判断** | `节点名.字段 === 'value'` | `Branch.status === 'active'` |
| **不等判断** | `节点名.字段 !== 'value'` | `LLM.type !== 'error'` |
| **数值比较** | `节点名.字段 > 数值` | `Score.value >= 60` |
| **长度比较** | `节点名.字段.length > 数值` | `Input.text.length > 10` |

### 安全求值机制

```
// 预编译正则表达式（模块级别，避免重复创建）
const INCLUDES_PATTERN = /^([...]).includes\(['"](.*)['"]\)$/;
const COMPARISON_PATTERN = /^([...])([>=<])(\d+)$/;

function safeEvaluateCondition(condition, context): boolean {
  // 1. 构建节点查找 Map（O(1) 查找）
  const lookupMap = buildNodeLookupMap(context);
  
  // 2. 白名单模式匹配
  const includesMatch = condition.match(INCLUDES_PATTERN);
  if (includesMatch) {
    // 安全执行 includes 检查
    return value.includes(searchStr);
  }
  
  // 3. 不支持的表达式 → 返回 false + 警告
  console.warn('Unsupported condition format:', condition);
  return false;
}
```

### 分支路径控制

```
graph TD
    A[上游节点] --> B[Branch]
    B -->|conditionResult=true| C[TRUE 路径]
    B -->|conditionResult=false| D[FALSE 路径]
    C --> E[下游节点 A]
    D --> F[下游节点 B]
    E --> G[汇合节点]
    F --> G
```

**路径阻塞机制**：
1. Branch 节点执行后返回 `conditionResult: boolean`
2. 根据结果确定未选中的 handle（`true` → 阻塞 `false` 路径）
3. 使用 `getDescendants()` 获取未选中路径的所有下游节点
4. 将这些节点加入 `blockedNodes` 集合，后续层级执行时跳过

```
if (node.type === 'branch' && result) {
  const conditionResult = !!result.conditionResult;
  const notTakenHandle = conditionResult ? 'false' : 'true';
  
  // 获取并阻塞未选中分支的所有下游
  const blockedDescendants = getDescendants(nodeId, edges, notTakenHandle);
  blockedDescendants.forEach(id => blockedNodes.add(id));
}
```

---

## 🔄 循环依赖检测

系统使用 DFS 算法检测 DAG 中的循环，定义于 [cycleDetection.ts](file:///Users/jasperlin/Desktop/product/flash-flow-saas/flash-flow/src/store/utils/cycleDetection.ts)。

```
function hasCycle(nodeId, nodes, edges, visited = new Set(), stack = new Set()): boolean {
  if (stack.has(nodeId)) return true;   // 当前路径中已存在 → 循环
  if (visited.has(nodeId)) return false; // 已访问过且无循环
  
  visited.add(nodeId);
  stack.add(nodeId);  // 加入当前路径栈
  
  const outgoers = getOutgoers({ id: nodeId }, nodes, edges);
  for (const out of outgoers) {
    if (hasCycle(out.id, nodes, edges, visited, stack)) return true;
  }
  
  stack.delete(nodeId);  // 回溯时移出路径栈
  return false;
}
```

**执行时机**：工作流执行前，对每个节点进行循环检测。

**性能优化**：
- **visited 剪枝**: 已检测过的无环子图不会重复检测
- **时间复杂度**: O(V + E)，其中 V 是节点数，E 是边数
- **空间复杂度**: O(V)，用于存储 visited 和 stack 集合

**循环示例**：
```
A → B → C → A  (循环依赖，被检测到)
A → B → C → D  (正常 DAG，通过检测)
```

---

## 🔍 节点解析工具

定义于 [sourceResolver.ts](file:///Users/jasperlin/Desktop/product/flash-flow-saas/flash-flow/src/store/utils/sourceResolver.ts)，提供高性能的节点查找能力。

### 节点索引结构

```
export interface NodeIndex {
    byId: Map<string, AppNode>;     // nodeId → node (O(1) 查找)
    byLabel: Map<string, AppNode>;  // label → node (O(1) 查找)
}

/**
 * 构建节点查找索引
 * O(n) 构建，支持 O(1) 查找
 */
export function buildNodeIndex(nodes: AppNode[]): NodeIndex {
    const byId = new Map<string, AppNode>();
    const byLabel = new Map<string, AppNode>();

    for (const node of nodes) {
        byId.set(node.id, node);
        const label = node.data?.label as string | undefined;
        if (label) {
            byLabel.set(label, node);
        }
    }

    return { byId, byLabel };
}
```

### 变量引用解析

```
/**
 * 从变量引用中提取节点 ID
 * 输入: "{{LLM1.response}}" 或 "{{节点名.字段}}"
 * 输出: 对应的 nodeId 或 null
 */
export function resolveSourceNodeId(
    value: string,
    nodes: AppNode[]
): string | null {
    // 提取 {{xxx}} 中的变量名
    const match = value.match(/\{\{(.+?)\}\}/);
    if (!match) return null;

    const varPath = match[1]; // 例如 "LLM1.response" 或 "response"

    // 尝试匹配 "节点名.字段" 格式
    const dotIndex = varPath.indexOf('.');
    if (dotIndex > 0) {
        const nodeLabel = varPath.substring(0, dotIndex);
        // 先匹配 label，再匹配 ID
        const matchedNode = nodes.find(n =>
            (n.data?.label as string) === nodeLabel ||
            n.id === nodeLabel
        );
        if (matchedNode) return matchedNode.id;
    }

    // 尝试直接匹配节点 label 或 ID
    const directMatch = nodes.find(n =>
        (n.data?.label as string) === varPath ||
        n.id === varPath
    );
    if (directMatch) return directMatch.id;

    return null;
}
```

**使用场景**：
- Output 节点的 `inputMappings.sources` 解析
- 流式输出源节点定位（`initSegmentedStreaming`, `initSelectStreaming`）

---

## 🔧 节点执行器架构

所有节点执行器统一由 [NodeExecutorFactory](file:///Users/jasperlin/Desktop/product/flash-flow-saas/flash-flow/src/store/executors/NodeExecutorFactory.ts) 管理：

```
// src/store/executors/NodeExecutorFactory.ts

export class NodeExecutorFactory {
    private static executors: Record<NodeKind, NodeExecutor> = {
        input: new InputNodeExecutor(),
        llm: new LLMNodeExecutor(),
        rag: new RAGNodeExecutor(),
        output: new OutputNodeExecutor(),
        branch: new BranchNodeExecutor(),
        tool: new ToolNodeExecutor(),
        imagegen: new ImageGenNodeExecutor(),
    };

    static getExecutor(nodeType: NodeKind): NodeExecutor {
        const executor = this.executors[nodeType];
        if (!executor) {
            throw new Error(`No executor found for node type: ${nodeType}`);
        }
        return executor;
    }
}
```

### 执行器调用方式

```
// executionActions.ts - executeSingleNode()

const executor = NodeExecutorFactory.getExecutor(node.type);
const { output, executionTime } = await executor.execute(node, upstreamContext);
```

### 核心执行器列表

| 执行器类 | 节点类型 | 主要功能 | 文件路径 |
|---------|---------|---------|----------|
| `InputNodeExecutor` | `input` | 提取用户输入（`user_input`, `files`, `formData`） | `InputNodeExecutor.ts` |
| `LLMNodeExecutor` | `llm` | 调用 LLM，支持流式、JSON 模式、对话记忆 | `LLMNodeExecutor.ts` |
| `RAGNodeExecutor` | `rag` | 双模式引擎：文件搜索 (Static) / 多模态理解 (Variable) | `RAGNodeExecutor.ts` |
| `OutputNodeExecutor` | `output` | 4 种模式（direct/select/merge/template）+ 附件解析 | `OutputNodeExecutor.ts` |
| `BranchNodeExecutor` | `branch` | 安全表达式求值 + 上游数据透传 | `BranchNodeExecutor.ts` |
| `ToolNodeExecutor` | `tool` | 工具执行 + Zod Schema 验证 | `ToolNodeExecutor.ts` |
| `ImageGenNodeExecutor` | `imagegen` | 文生图/图生图 + 配额检查 + 变量替换 | `ImageGenNodeExecutor.ts` |

---

## 🧩 核心节点特殊机制

### 1. RAG 节点双模式引擎

RAG 节点根据 `fileMode` 和输入文件来源自动选择执行策略：

| 模式 | 触发条件 | 底层机制 | 适用场景 |
|------|---------|---------|---------|
| **变量模式 (Variable)** | `fileMode='variable'` 或 引用了上游文件 | **Multimodal API** | 处理上游动态传入的文件（图片/PDF），进行即时多模态理解 |
| **静态模式 (Static)** | `fileMode='static'` 且无动态文件引用 | **File Search Store** | 在预先构建的知识库中进行大规模文档检索 |

**多文件支持**：支持 `files`, `files2`, `files3` 三个动态文件槽位，执行时会自动合并。

### 2. ImageGen 节点完整能力

支持文生图与图生图，完整参数映射：

- **基础参数**: `prompt` (支持变量), `negativePrompt`
- **模型参数**: `imageSize`, `cfg` (或 `guidanceScale`), `numInferenceSteps`
- **图生图 (Reference Image)**:
  - **Static**: 使用上传的固定 URL (`referenceImageUrl`)
  - **Variable**: 引用上游图片变量 (`referenceImageVariable`)，支持 `{{xx.imageUrl}}`

### 3. LLM 节点高级配置

- **JSON 模式**: 配置 `responseFormat: 'json_object'` 可强制输出标准 JSON。
- **对话记忆**: 
  - `enableMemory`: 开启后自动携带最近 `memoryMaxTurns` 轮的历史对话。
  - `sessionId`: 通过 FlowContext 传递，实现跨节点/跨轮次的记忆隔离。

---


## 🛡️ 安全与容错机制

### 1. 参数验证

| 节点类型 | 验证机制 | 失败处理 |
|---------|---------|----------|
| **Input** | 必填项检查（`runFlow` 编排层执行） | 打开 `InputPromptDialog` 提示输入 |
| **LLM** | `systemPrompt` 非空、`temperature` 范围验证 | 抛出异常，设置 `executionStatus: "error"` |
| **RAG** | `files` 数组非空、`fileSearchStoreName` 已配置 | 抛出异常 |
| **Tool** | Zod Schema 二次验证（在工具定义中声明） | 验证失败抛出详细错误信息 |
| **Branch** | 白名单表达式匹配（非白名单返回 false + 警告） | 控制台警告 + 默认 false |
| **Output** | 模式与配置一致性检查（`sources` 非空、`template` 存在） | 抛出配置错误异常 |
| **ImageGen** | 配额检查 + 提示词非空 | 抛出配额不足或参数缺失异常 |

### 2. 敏感数据过滤

| 场景 | 过滤规则 | 实现位置 |
|------|---------|----------|
| **Branch 透传** | 过滤 `_` 开头字段（`_meta` 等） | `BranchNodeExecutor.ts` L233-237 |
| **FlowContext 访问** | `getUpstreamEntries` 过滤 `_meta` | `contextUtils.ts` L11-28 |
| **变量收集** | `flattenObject` 跳过 `_` 开头的键 | `variableUtils.ts` L29 |
| **文本提取** | 过滤 `conditionResult`, `passed`, `value` | `contextUtils.ts` L46, L84-88 |

### 3. 执行完整性检查

防止执行期间节点被删除导致状态混乱：

```
// executionActions.ts - runFlow()

const initialNodeIds = new Set(nodes.map((n: AppNode) => n.id));

const checkFlowIntegrity = () => {
    const currentNodes = get().nodes;
    const currentIds = new Set(currentNodes.map((n: AppNode) => n.id));
    for (const id of initialNodeIds) {
        if (!currentIds.has(id)) {
            throw new Error(
                "Execution interrupted: Flow structure changed (node deleted)."
            );
        }
    }
};

// 在关键节点执行前调用
for (let level = 0; level <= maxLevel; level++) {
    checkFlowIntegrity();  // 检查完整性
    // ...执行节点
}
```

### 4. 错误传播机制

```
// 收集层级内的执行错误
const executionErrors: { nodeId: string; error: Error }[] = [];

const results = await Promise.allSettled(
    executableNodes.map(nodeId => executeNodeAndHandleBranch(nodeId))
);

results.forEach((result, index) => {
    if (result.status === 'rejected') {
        executionErrors.push({
            nodeId: executableNodes[index],
            error: result.reason instanceof Error 
                ? result.reason 
                : new Error(String(result.reason))
        });
    }
});

// 如果有错误，中止后续层级执行
if (executionErrors.length > 0) {
    const errorMessages = executionErrors
        .map(e => `${e.nodeId}: ${e.error.message}`)
        .join('; ');
    throw new Error(`节点执行失败: ${errorMessages}`);
}
```

---

## 🎚️ 流式输出模式

Output 节点支持三种流式输出模式，由 `inputMappings.mode` 决定：

| 模式 | 说明 | 初始化函数 | 适用场景 |
|------|------|-------------|----------|
| `single` | 单源直接输出（默认） | 无（默认状态） | 单个 LLM 节点直连 Output |
| `segmented` | 分段流式（merge 模式） | `initSegmentedStreaming(sourceIds)` | 多个 LLM 并行执行，顺序显示每段结果 |
| `select` | 首字锁定（select 模式） | `initSelectStreaming(sourceIds)` | 分支场景，只显示首个输出的分支 |

### Single 模式（默认）

```
// 直接设置 streamingText
// flowStore 中的 appendStreamingText 方法会自动追加内容
appendStreamingText(chunk);  // chunk by chunk 追加
```

### Segmented 模式（merge）

```
// 初始化：为每个源创建一个 segment
initSegmentedStreaming(sourceIds: string[]) {
    set({
        streamingMode: 'segmented',
        streamingSegments: sourceIds.map(id => ({
            sourceId: id,
            content: '',
            status: 'waiting'
        }))
    });
}

// 追加内容：更新对应 segment
appendToSegment(sourceId: string, chunk: string) {
    set((state) => ({
        streamingSegments: state.streamingSegments.map(seg =>
            seg.sourceId === sourceId
                ? { ...seg, content: seg.content + chunk, status: 'streaming' }
                : seg
        )
    }));
}

// 完成标记
completeSegment(sourceId: string) {
    set((state) => ({
        streamingSegments: state.streamingSegments.map(seg =>
            seg.sourceId === sourceId
                ? { ...seg, status: 'completed' }
                : seg
        )
    }));
}
```

### Select 模式（首字锁定）

```
// 初始化
initSelectStreaming(sourceIds: string[]) {
    set({
        streamingMode: 'select',
        selectSourceIds: sourceIds,
        lockedSourceId: null  // 尚未锁定
    });
}

// 尝试锁定源（首个输出内容的节点会被锁定）
tryLockSource(sourceId: string): boolean {
    const currentState = get();
    
    // 如果尚未锁定，锁定该源
    if (currentState.lockedSourceId === null) {
        set({ lockedSourceId: sourceId });
        return true;  // 成功锁定
    }
    
    // 已锁定同一源，允许继续输出
    return currentState.lockedSourceId === sourceId;
}
```

**使用场景示例**：

```
# Segmented 模式（merge）
Input → LLM1 (总结)
      → LLM2 (翻译) 
      → Output (merge: [LLM1, LLM2])
      
结果: 依次显示 LLM1 的流式输出，然后显示 LLM2 的流式输出

# Select 模式
Input → Branch
      ├─ true → LLM1
      └─ false → LLM2
      → Output (select: [LLM1, LLM2])
      
结果: 只显示首个开始输出的 LLM 节点的内容
```

---

## ⚠️ 常见错误与处理

| 错误场景 | 错误信息 | 预防措施 | 代码位置 |
|---------|---------|---------|----------|
| **循环依赖** | `"检测到循环依赖，无法执行工作流"` | 执行前 DFS 检测 | `executionActions.ts` L122-130 |
| **并发执行** | `"流程正在执行中，请等待完成后再试"` | `_executionLock` 执行锁 | `executionActions.ts` L170-174 |
| **节点删除** | `"Execution interrupted: Flow structure changed"` | `checkFlowIntegrity()` 检查 | `executionActions.ts` L233-241 |
| **非法表达式** | 控制台警告 + 默认 false | 白名单模式匹配 | `BranchNodeExecutor.ts` L158-160 |
| **变量未找到** | `"[PromptParser] 未找到变量: xxx"` | 控制台日志 + 替换为空 | `promptParser.ts` L73 |
| **输入缺失** | 打开 `InputPromptDialog` | `runFlow` 开始时验证 | `executionActions.ts` L134-167 |
| **Output 配置错** | `"Output 节点配置错误: xxx"` | 模式与 sources/template 一致性检查 | `OutputNodeExecutor.ts` L164-211 |

---

## 📁 核心代码文件索引

### 类型定义
- [flow.ts](file:///Users/jasperlin/Desktop/product/flash-flow-saas/flash-flow/src/types/flow.ts) - `FlowContext`, `FlowContextMeta`, `AppNode`, `AppEdge`, `FlowState` 等核心类型

### 执行引擎
- [executionActions.ts](file:///Users/jasperlin/Desktop/product/flash-flow-saas/flash-flow/src/store/actions/executionActions.ts) - `runFlow`, `runNode`, `executeSingleNode` 执行主入口
- [parallelExecutionUtils.ts](file:///Users/jasperlin/Desktop/product/flash-flow-saas/flash-flow/src/store/utils/parallelExecutionUtils.ts) - 拓扑层级计算、并行执行工具
- [cycleDetection.ts](file:///Users/jasperlin/Desktop/product/flash-flow-saas/flash-flow/src/store/utils/cycleDetection.ts) - 循环依赖检测算法

### 变量系统
- [variableUtils.ts](file:///Users/jasperlin/Desktop/product/flash-flow-saas/flash-flow/src/store/executors/utils/variableUtils.ts) - `collectVariables`, `flattenObject` 变量收集与展开
- [promptParser.ts](file:///Users/jasperlin/Desktop/product/flash-flow-saas/flash-flow/src/lib/promptParser.ts) - `replaceVariables`, `extractVariables` 变量替换逻辑
- [sourceResolver.ts](file:///Users/jasperlin/Desktop/product/flash-flow-saas/flash-flow/src/store/utils/sourceResolver.ts) - 节点解析与索引工具

### 上下文管理
- [contextUtils.ts](file:///Users/jasperlin/Desktop/product/flash-flow-saas/flash-flow/src/store/executors/contextUtils.ts) - 上下文数据访问工具函数

### 节点执行器
- [NodeExecutorFactory.ts](file:///Users/jasperlin/Desktop/product/flash-flow-saas/flash-flow/src/store/executors/NodeExecutorFactory.ts) - 执行器工厂
- [InputNodeExecutor.ts](file:///Users/jasperlin/Desktop/product/flash-flow-saas/flash-flow/src/store/executors/InputNodeExecutor.ts) - Input 节点执行器
- [OutputNodeExecutor.ts](file:///Users/jasperlin/Desktop/product/flash-flow-saas/flash-flow/src/store/executors/OutputNodeExecutor.ts) - Output 节点执行器（4 种模式）
- [BranchNodeExecutor.ts](file:///Users/jasperlin/Desktop/product/flash-flow-saas/flash-flow/src/store/executors/BranchNodeExecutor.ts) - Branch 节点执行器（安全表达式求值）
- [LLMNodeExecutor.ts](file:///Users/jasperlin/Desktop/product/flash-flow-saas/flash-flow/src/store/executors/LLMNodeExecutor.ts) - LLM 节点执行器（流式输出 + 记忆）
- [RAGNodeExecutor.ts](file:///Users/jasperlin/Desktop/product/flash-flow-saas/flash-flow/src/store/executors/RAGNodeExecutor.ts) - RAG 节点执行器
- [ToolNodeExecutor.ts](file:///Users/jasperlin/Desktop/product/flash-flow-saas/flash-flow/src/store/executors/ToolNodeExecutor.ts) - Tool 节点执行器
- [ImageGenNodeExecutor.ts](file:///Users/jasperlin/Desktop/product/flash-flow-saas/flash-flow/src/store/executors/ImageGenNodeExecutor.ts) - ImageGen 节点执行器（图片生成 + 配额检查）

---

## 📈 性能优化策略

### 1. 节点索引优化

```
// executionActions.ts - runFlow()
// 使用 Map 存储静态节点数据，避免重复查找
const nodeMap = new Map<string, AppNode>(nodes.map((n: AppNode) => [n.id, n]));

// O(1) 查找节点类型
const node = nodeMap.get(nodeId);
```

### 2. 正则预编译

```
// BranchNodeExecutor.ts - 模块级别预编译
const INCLUDES_PATTERN = /^([a-zA-Z\u4e00-\u9fa5_][\w\u4e00-\u9fa5]*\.[\w.]+)\.includes\(['"](.+)['"]\)$/;
// 避免每次执行时重复编译
```

### 3. 记忆化优化

```
// parallelExecutionUtils.ts - calculateTopologicalLevels()
const calculateLevel = (nodeId: string): number => {
    // 已计算过，直接返回
    if (levels.has(nodeId)) {
        return levels.get(nodeId)!;
    }
    // ... 计算逻辑
};
```

### 4. 并行执行优化

```
// executionActions.ts
// 同层级节点并行执行，大幅缩短执行时间
const results = await Promise.allSettled(
    executableNodes.map(nodeId => executeNodeAndHandleBranch(nodeId))
);
```

### 5. 剪枝优化

```
// cycleDetection.ts
// 已检测过的无环子图不会重复检测
if (visited.has(nodeId)) return false;
```

---

## 🔍 调试工具

### 1. 单节点调试

```
// 通过 runNode 函数调试单个节点
runNode(nodeId, mockInputData?: Record<string, unknown>)

// LLM 节点：打开 LLMDebugDialog
get().openLLMDebugDialog(nodeId);

// RAG 节点：打开 RAGDebugDialog
get().openRAGDebugDialog(nodeId);

// Tool 节点：打开 ToolDebugDialog
get().openToolDebugDialog(nodeId);

// ImageGen 节点：打开 ImageGenDebugDialog
get().openImageGenDebugDialog(nodeId);
```

### 2. 上下文查看

````
const context = useFlowStore.getState().flowContext;
console.log(context);

// 查看节点标签映射
console.log(context._meta?.nodeLabels);
```

### 3. 日志输出

- **变量未找到**: `[PromptParser] 未找到变量: xxx`
- **非法表达式**: `[BranchNodeExecutor] Unsupported condition format: xxx`
- **执行错误**: `executionError` 状态字段中的错误信息

---

## 🚀 最佳实践

### 1. 变量引用
- **优先使用节点 label**: `{{获取时间.formatted}}` 比 `{{formatted}}` 更清晰
- **避免同名冲突**: 多个 LLM 节点都输出 `response` 时，使用带前缀的引用
- **检查变量存在**: 查看控制台警告，确保变量已定义

### 2. 分支表达式
- **使用节点 label**: `用户输入.user_input.includes('查询')` 比 `input_xxx.user_input` 更可读
- **避免复杂表达式**: 只使用白名单支持的格式
- **测试表达式**: 使用 `runNode` 调试分支逻辑

### 3. 流程设计
- **避免循环**: 保持 DAG 结构
- **合理分层**: 充分利用并行执行，避免不必要的依赖
- **限制深度**: 避免过长的依赖链

### 4. 错误处理
- **提供默认值**: Input 节点配置 `defaultValue`
- **验证输入**: 在执行前检查必填项
- **处理异常**: 捕获并展示有意义的错误信息
