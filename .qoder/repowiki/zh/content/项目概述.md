...
<cite>
**本文档中引用的文件**  
- [README.md](file://README.md) - *项目说明文件*
- [package.json](file://package.json) - *项目依赖与脚本配置*
- [layout.tsx](file://src/app/layout.tsx) - *应用布局组件*
- [page.tsx](file://src/app/page.tsx) - *首页组件*
- [flowStore.ts](file://src/store/flowStore.ts) - *核心状态管理*
- [flow.ts](file://src/types/flow.ts) - *流程数据类型定义*
- [flowAPI.ts](file://src/services/flowAPI.ts) - *流程数据服务接口*
- [supabase.ts](file://src/lib/supabase.ts) - *Supabase数据库客户端*
- [CustomNode.tsx](file://src/components/flow/CustomNode.tsx) - *自定义节点渲染组件*
- [nodeActions.ts](file://src/store/actions/nodeActions.ts) - *节点操作逻辑*
- [executionActions.ts](file://src/store/actions/executionActions.ts) - *执行控制逻辑*
- [builder/page.tsx](file://src/app/builder/page.tsx) - *构建器页面入口*
- [database.ts](file://src/types/database.ts) - *数据库类型定义*
- [copilotActions.ts](file://src/store/actions/copilotActions.ts) - *AI辅助生成逻辑*
- [plan/route.ts](file://src/app/api/plan/route.ts) - *AI流程生成API*
- [run-node/route.ts](file://src/app/api/run-node/route.ts) - *节点执行API*
- [NodeExecutorFactory.ts](file://src/store/executors/NodeExecutorFactory.ts) - *节点执行器工厂*
- [BaseNodeExecutor.ts](file://src/store/executors/BaseNodeExecutor.ts) - *节点执行器基类*
</cite>
...
**图表来源**  
- [src/app/page.tsx](file://src/app/page.tsx) - *首页组件*  
- [src/app/builder/page.tsx](file://src/app/builder/page.tsx) - *构建器页面入口*  
- [src/store/flowStore.ts](file://src/store/flowStore.ts) - *核心状态管理*  
- [src/services/flowAPI.ts](file://src/services/flowAPI.ts) - *流程数据服务接口*  
- [src/lib/supabase.ts](file://src/lib/supabase.ts) - *Supabase数据库客户端*
...
- **AI驱动的流程生成**：通过 `/api/plan` 接口，用户输入自然语言描述，系统可自动生成符合规范的流程节点与连接，实现智能化辅助构建。
- **执行引擎与节点调度**：基于 `NodeExecutorFactory` 的执行器模式，不同类型节点由对应的执行器处理，支持LLM调用、RAG检索、工具调用等复杂操作。

架构上，项目采用“状态驱动UI”的设计模式，所有用户交互最终转化为对 Zustand store 的状态变更，UI 组件监听状态变化并重新渲染。这种模式简化了数据流管理，提升了可预测性和可测试性。

**节来源**  
- [src/types/flow.ts](file://src/types/flow.ts) - *流程数据类型定义*  
- [src/store/flowStore.ts](file://src/store/flowStore.ts) - *核心状态管理*  
- [src/components/flow/CustomNode.tsx](file://src/components/flow/CustomNode.tsx) - *自定义节点渲染组件*  
- [src/app/api/plan/route.ts](file://src/app/api/plan/route.ts) - *AI流程生成API*  
- [src/store/executors/NodeExecutorFactory.ts](file://src/store/executors/NodeExecutorFactory.ts) - *节点执行器工厂*
...
**图表来源**  
- [src/store/flowStore.ts](file://src/store/flowStore.ts) - *核心状态管理*  
- [src/store/actions/nodeActions.ts](file://src/store/actions/nodeActions.ts) - *节点操作逻辑*  
- [src/store/actions/executionActions.ts](file://src/store/actions/executionActions.ts) - *执行控制逻辑*  
- [src/types/flow.ts](file://src/types/flow.ts) - *流程数据类型定义*
...
执行引擎采用策略模式，`NodeExecutorFactory` 根据节点类型（`input`, `llm`, `rag`, `output`, `branch`, `tool`）返回对应的执行器（`InputNodeExecutor`, `LLMNodeExecutor` 等），实现解耦和可扩展性。

**节来源**  
- [src/app/api/run-node/route.ts](file://src/app/api/run-node/route.ts) - *节点执行API*  
- [src/lib/supabase.ts](file://src/lib/supabase.ts) - *Supabase数据库客户端*  
- [src/types/database.ts](file://src/types/database.ts) - *数据库类型定义*  
- [src/store/executors/NodeExecutorFactory.ts](file://src/store/executors/NodeExecutorFactory.ts) - *节点执行器工厂*  
- [src/store/executors/BaseNodeExecutor.ts](file://src/store/executors/BaseNodeExecutor.ts) - *节点执行器基类*
