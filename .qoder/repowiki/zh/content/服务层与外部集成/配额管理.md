...
## 更新摘要
**变更内容**
- 新增了边缘运行时配额管理功能，增强了服务器端的配额检查和扣减逻辑
- 在API路由中实现了服务器端配额验证，提高了系统的安全性
- 更新了配额管理架构，增加了服务端与客户端的协同机制
...
配额管理贯穿"前端状态管理 + 后端服务 + 数据库"的全链路，既保证用户体验（低延迟、友好提示），又确保系统资源可控（防滥用、并发安全）。本次更新集成了新的边缘运行时配额管理功能，增强了服务器端的配额检查和扣减逻辑，通过在API路由层直接进行配额验证，提高了系统的安全性和可靠性。
...
- 边缘运行时配额工具：src/lib/quotaEdge.ts
- 边缘运行时认证工具：src/lib/authEdge.ts
...
- [quotaEdge.ts](file://src/lib/quotaEdge.ts#L1-L160)
- [authEdge.ts](file://src/lib/authEdge.ts#L1-L95)
...
- 边缘运行时配额工具：提供服务器端配额检查和扣减功能，增强API安全性
- 边缘运行时认证工具：提供边缘运行时环境下的用户认证功能
...
- 新增边缘运行时配额检查，在API路由层直接进行服务器端配额验证
...
participant EDGE_Q as "quotaEdge"
participant EDGE_A as "authEdge"
...
API->>EDGE_A : 验证用户身份
EDGE_A->>API : 返回用户信息
API->>EDGE_Q : checkQuotaOnServer
EDGE_Q->>DB : 读取配额
DB-->>EDGE_Q : 返回配额
EDGE_Q-->>API : 允许/拒绝
alt 允许
API->>业务逻辑 : 执行请求
API->>EDGE_Q : incrementQuotaOnServer
EDGE_Q->>DB : 乐观锁更新计数
DB-->>EDGE_Q : 返回结果
EDGE_Q-->>API : 返回成功
else 拒绝
...
class quotaEdge {
+checkQuotaOnServer(request, userId, type) QuotaCheckResult
+incrementQuotaOnServer(request, userId, type) boolean
+quotaExceededResponse(used, limit, quotaLabel) Response
}
UserQuota <.. quotaService : "读写"
UserQuota <.. quotaEdge : "读写"
QuotaType <.. quotaService : "使用"
QuotaType <.. quotaEdge : "使用"
QuotaCheckResult <.. quotaService : "返回"
QuotaCheckResult <.. quotaEdge : "返回"
```
...
### 边缘运行时配额工具：quotaEdge
新增的边缘运行时配额工具提供了服务器端的配额管理功能，增强了API的安全性。

**核心功能：**
- `checkQuotaOnServer`：在服务器端检查用户配额，返回配额状态
- `incrementQuotaOnServer`：在服务器端增量扣减配额，使用乐观锁防止并发问题
- `quotaExceededResponse`：创建429响应，用于配额不足的情况

**实现特点：**
- 使用边缘运行时客户端与Supabase交互
- 采用"fail closed"策略，配额检查失败时默认拒绝访问
- 在增量扣减时使用乐观锁和重试机制，确保数据一致性
- 与authEdge工具协同工作，先验证用户身份再进行配额检查

```mermaid
flowchart TD
Start(["进入 checkQuotaOnServer"]) --> Auth["创建边缘客户端"]
Auth --> Load["加载用户配额"]
Load --> Exists{"存在配额记录？"}
Exists --> |否| ReturnZero["返回不允许<br/>used=0, limit=0, remaining=0"]
Exists --> |是| Map["类型安全映射 used/limit"]
...
图表来源
- [quotaEdge.ts](file://src/lib/quotaEdge.ts#L21-L54)
...
### 后端 API 与配额的关系
- /api/run-node：执行单次 LLM 请求，现在在边缘运行时层进行配额检查和扣减
- /api/modify-flow：AI 修改工作流指令生成，现在在边缘运行时层进行配额检查和扣减

**更新内容：**
- 在API路由中集成了服务器端配额检查，增强了安全性
- 使用`checkQuotaOnServer`在请求处理前验证配额
- 使用`incrementQuotaOnServer`在业务逻辑成功后扣减配额
- 使用`quotaExceededResponse`返回标准化的429响应
...
- 组件耦合
  ...
  - API路由依赖quotaEdge和authEdge，实现服务器端的配额管理和用户认证
- 外部依赖
...
API_RUN["run-node/route.ts"] --> EDGE_Q["quotaEdge.ts"]
API_MOD["modify-flow/route.ts"] --> EDGE_Q
EDGE_Q --> DB_Q
EDGE_Q --> EDGE_A["authEdge.ts"]
```
...
- 服务器端验证：在API层进行配额检查，减少无效请求的处理开销
...
- 服务器端配额检查失败
  - 现象：API返回429状态码，提示配额已用完
  - 排查：检查quotaEdge.ts中的checkQuotaOnServer实现；确认用户ID是否正确传递；查看边缘运行时日志
...
## 结论
本项目的配额管理通过"类型安全 + 乐观锁 + 前端拦截 + UI 反馈"的组合，实现了对 LLM 执行、Flow 生成与 App 交互的细粒度控制。本次更新集成了新的边缘运行时配额管理功能，增强了服务器端的配额检查和扣减逻辑，其优点在于：
- 明确的配额边界与清晰的错误提示
- 并发安全的增量扣减
- 低侵入的前端拦截，不影响后端 API 的职责划分
- 新增服务器端配额验证，提高了系统的安全性
