...
graph TB
subgraph "前端界面层"
A[Flows页面] --> B[FlowCard组件]
C[Builder页面] --> D[FlowCanvas组件]
end
subgraph "状态管理层"
E[flowStore] --> F[zustand状态管理]
end
subgraph "服务层"
G[flowAPI] --> H[Supabase客户端]
end
subgraph "数据层"
H --> I[Supabase数据库]
H --> J[Storage Bucket]
...
API->>Supabase : 查询flows表
Supabase-->>API : 返回原始数据
API->>Validation : 验证数据结构
Validation->>Validation : 检查nodes/edges
Validation->>Validation : 处理损坏数据
Validation-->>API : 返回标准化数据
...
```mermaid
flowchart TD
Start([开始获取工作流]) --> QueryDB["查询Supabase flows表"]
QueryDB --> CheckError{"是否有错误?"}
CheckError --> |是| LogError["记录错误日志"]
CheckError --> |否| CheckData{"是否有数据?"}
CheckData --> |否| ReturnNull["返回null"]
CheckData --> |是| ValidateData["验证数据结构"]
ValidateData --> BuildResponse["构建响应对象"]
BuildResponse --> End([返回FlowRecord])
LogError --> ReturnNull
```
...
Store->>Timer : 设置800ms定时器
Store->>Store : 更新saveStatus为"saving"
Timer->>Timer : 等待800ms
...
DB-->>API : 返回结果
API-->>Store : 返回flowId
Store->>Store : 更新saveStatus为"saved"
```
...
```mermaid
flowchart TD
Start([接收数据]) --> ParseData["解析JSON数据"]
ParseData --> CheckType{"数据类型检查"}
CheckType --> |非对象| DegradedMode["降级模式"]
CheckType --> |对象| ValidateFields["验证必需字段"]
ValidateFields --> CheckNodes{"nodes字段检查"}
CheckNodes --> |缺失| SetEmptyNodes["设置空nodes数组"]
CheckNodes --> |存在| CheckEdges{"edges字段检查"}
CheckEdges --> |缺失| SetEmptyEdges["设置空edges数组"]
CheckEdges --> |存在| Success["验证成功"]
SetEmptyNodes --> Success
SetEmptyEdges --> Success
DegradedMode --> LogError["记录错误日志"]
LogError --> ReturnDefault["返回默认结构"]
Success --> ReturnData["返回有效数据"]
```
