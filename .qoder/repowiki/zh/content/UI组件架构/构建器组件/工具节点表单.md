...
- [types.ts](file://src/components/builder/context-hud/types.ts)
- [UpstreamInputsSection.tsx](file://src/components/builder/context-hud/NodeIOSection/UpstreamInputsSection.tsx)
</cite>

## 更新摘要
**变更内容**
- 更新“工具节点表单渲染”部分，以反映基于 `TOOL_IO_DEFINITIONS` 和 `ToolInputField` 类型的动态智能UI支持
- 新增关于条件显示、字段类型（枚举、数字）和依赖字段的说明
- 更新架构总览和详细组件分析中的数据流描述
- 移除过时的“移除动态输入字段渲染”说明
...
- 上下文 HUD 与变量引用：NodeIOSection.tsx、constants.ts、types.ts、UpstreamInputsSection.tsx
...
D["types.ts<br/>工具输入字段类型"]
E["UpstreamInputsSection.tsx<br/>上游输入配置"]
end
subgraph "运行期"
F["ToolNodeExecutor.ts<br/>执行器"]
G["BaseNodeExecutor.ts<br/>基类"]
H["tools.ts<br/>服务器动作"]
I["registry.ts<br/>工具注册与校验"]
end
subgraph "上下文与调试"
J["NodeIOSection.tsx<br/>上下文 HUD"]
K["ToolDebugDialog.tsx<br/>调试对话框"]
end
A --> I
B --> I
C --> J
...
图表来源
- [ToolNodeForm.tsx](file://src/components/builder/node-forms/ToolNodeForm.tsx#L1-L76)
- [ToolNode.tsx](file://src/components/flow/nodes/ToolNode.tsx#L1-L98)
- [constants.ts](file://src/components/builder/context-hud/constants.ts#L67-L169)
- [types.ts](file://src/components/builder/context-hud/types.ts#L31-L38)
- [UpstreamInputsSection.tsx](file://src/components/builder/context-hud/NodeIOSection/UpstreamInputsSection.tsx#L1-L181)
...
- 功能要点
  ...
  - 支持基于 `TOOL_IO_DEFINITIONS` 和 `ToolInputField` 类型的更动态和智能的UI，包括条件显示和字段类型（如枚举、数字）。
...
- 功能要点
  ...
  - 基于 `TOOL_IO_DEFINITIONS` 和 `ToolInputField` 实现动态输入字段渲染，支持条件显示（`dependsOn`）、字段类型（`type`）和枚举选项（`enumOptions`）。
...
## 结论
工具节点表单通过“Schema 驱动的动态表单 + 注册中心 + 执行器 + 服务器动作 + 上下文 HUD + 调试对话框”的完整链路，实现了高内聚、低耦合的工具扩展机制。新增工具只需在注册中心定义 Schema 与执行函数，即可自动获得表单渲染、调试与上下文变量支持。建议在新增工具时同步完善工具 IO 定义与描述，提升用户体验与可维护性。