...
<cite>
**本文档中引用的文件**
- [FlowCanvas.tsx](file://src/components/flow/FlowCanvas.tsx) - *可视化画布核心实现*
- [flowAPI.ts](file://src/services/flowAPI.ts) - *流程数据持久化服务*
- [planNormalizer.ts](file://src/store/utils/planNormalizer.ts) - *智能规划数据转换*
- [LaunchCard.tsx](file://src/components/builder/LaunchCard.tsx) - *节点启动机制实现*
- [flowStore.ts](file://src/store/flowStore.ts) - *全局状态管理*
- [layoutAlgorithm.ts](file://src/store/utils/layoutAlgorithm.ts) - *自动布局算法*
- [cycleDetection.ts](file://src/store/utils/cycleDetection.ts) - *循环检测逻辑*
- [CustomNode.tsx](file://src/components/flow/CustomNode.tsx) - *自定义节点渲染*
- [nodeActions.ts](file://src/store/actions/nodeActions.ts) - *节点操作动作*
- [edgeActions.ts](file://src/store/actions/edgeActions.ts) - *边操作动作*
- [executionActions.ts](file://src/store/actions/executionActions.ts) - *执行控制动作*
- [route.ts](file://src/app/api/run-node/route.ts) - *节点执行API*
- [route.ts](file://src/app/api/modify-flow/route.ts) - *流程修改API*
- [page.tsx](file://src/app/builder/page.tsx) - *构建器页面逻辑*
