...
3. **禁止多模态输入**: 系统目前仅支持文本交互。严禁将图片 ({{Input.files[0].url}})、视频或音频传给 LLM 节点，即使模型 ID 看起来支持多模态。
4. **禁止死锁合并**: 严禁将互斥的分支路径（Branch 的 true/false）直接合并到同一个标准节点，仅允许在 Output 节点汇聚。
5. **禁止中文变量名**: 所有的变量 ID (formData 字段、LLM JSON key) 必须为英文。
